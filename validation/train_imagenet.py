...
parser.add_argument('-a', '--arch', metavar='ARCH', default='nsganet',
                    # choices=model_names,
...
def main():
    ...
    args.save = 'train-ImageNet-{}-{}'.format(args.save, time.strftime("%Y%m%d-%H%M%S"))
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # use custom models
    if args.arch == 'nsganet':
        genome = genotypes.NSGANet
        model = micro_models.NetworkImageNet(46, 1000, 14, True, genome, SE=True)
    else:
    ...
    train_dataset = datasets.ImageFolder(
        ...
        transforms.Compose([
            ...
            transforms.RandomResizedCrop(224, scale=(0.2, 1.0)),  # https://github.com/tonylins/pytorch-mobilenet-v2
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        acc1, acc5, _ = validate(val_loader, model, criterion, args)
        logging.info('valid_acc %f %f', acc1, acc5)
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if i % args.print_freq == 0:
            # progress.print(i)
            logging.info('train %04d/%04d %e %f %f', i, len(train_loader),
...
def validate(val_loader, model, criterion, args):
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            if i % args.print_freq == 0:
                # progress.print(i)
                logging.info('valid %03d/%03d %e %f %f', i, len(val_loader),
                             losses.avg, top1.avg, top5.avg)
